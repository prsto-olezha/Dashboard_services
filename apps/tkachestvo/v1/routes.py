...
from .utils import tkachestvo_stats_aggregation, sum_tkachestvo_stats_aggregation, manual_write_data
...
tkachestvo_router_v1 = APIRouter(
    prefix="/v1", tags=['Tkachestvo']
)


@tkachestvo_router_v1.get('/get-stats', response_model=TkachestvoGetStatsResponse)
...
@tkachestvo_router_v1.post('/upload-data')
async def tkachestvo_upload_data(Tkachestvo: UploadFile, Weaving: UploadFile, date: dt.date = None, date_from: dt.datetime = None, date_to: dt.datetime = None):
    try:
        content1 = io.BytesIO(await Tkachestvo.read())
        content2 = io.BytesIO(await Weaving.read())
        if date_from != None and date_to != None:
            ...
            while current_date <= date_to:
                content1.seek(0)
                content2.seek(0)
                data = await tkachestvo_data(current_date.date(), [content1, content2])
                data_obj = await manual_write_data(current_date.date(), data)
                current_date += delta
        elif date != None:
            # Обрабатываем данные
            data = await tkachestvo_data(date, [content1, content2])
